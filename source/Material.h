...
#pragma region Material COOK TORRENCE
	...
	public:
		...
		ColorRGB Shade(const HitRecord& hitRecord = {}, const Vector3& l = {}, const Vector3& v = {}) override
		{
			...
			else f0 = ColorRGB{ 0.04f, 0.04f, 0.04f };

			Vector3 halfVector = (v + l).Normalized();
			ColorRGB fresel = BRDF::FresnelFunction_Schlick(halfVector, v, f0);
			float theD = BRDF::NormalDistribution_GGX(hitRecord.normal, halfVector, m_Roughness);
			float geometry = BRDF::GeometryFunction_Smith(hitRecord.normal, v, l, m_Roughness);
			ColorRGB specular{ (theD * fresel * geometry) *(1.f / (4 * (Vector3::Dot(v,hitRecord.normal) * Vector3::Dot(l,hitRecord.normal))))};
			...
			else kd = ColorRGB{ 1.f,1.f,1.f } - fresel;
